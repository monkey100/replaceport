...
//CHANGELOG
//Added the object parameter for the onOpen function to access plugin externally.
...
(function ($) {
	...
	$.Popup.settings = {
		'openBox'				: false,			// Opens the box as soon as it is defined
		...
		'reopenClick'			: true,				//Reopens the popup when the bound element is clciked after close
	...
	$.Popup.prototype = {
		init : function() {
			...
			//this is pulling the event out. but working the first time.
			if (this.opts.reopenClick)
			{
		...
		createBox : function()
		{
			...
			if ($("#" + this.opts.popupID).length === 0)
			{
				...
				if (this.opts.reopenClick)
				{
					//This has created a conflict with existing elements within the page so has been fully isolated.
		...
		{
			if (this.el.boxSrc == "undefined" || this.el.boxSrc.indexOf('.') == -1)
			{
				return false;
			}
...
//*!*This has been overridden to handle more dynamically built pages.
//*!*Will need to test all intances of this before declaring this change "safe".
// 				if (pluginInstance) {
// 
